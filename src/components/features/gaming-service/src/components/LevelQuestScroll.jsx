 ...
 *
 * Crop challenges are listed sequentially with LOCKED / AVAILABLE / COMPLETED
 * so every level plant is visible — only the current one is playable.
 */
export default function LevelQuestScroll({
  ...
  const tasks = useMemo(() => {
    ...
    if (animalTarget > 0 && animalName) {
      ...
      const cropsDone = Boolean(levelCropComplete);
      const animalDone =
        Boolean(levelAnimalComplete) || animalSold >= animalTarget;
      const animalLocked = !cropsDone && !animalDone;
      const animalStatus = animalDone
        ? 'COMPLETED'
        : animalLocked
          ? 'LOCKED'
          : 'AVAILABLE';
      ...
      list.push({
        ...
        detail: animalLocked
          ? 'Locked — finish every vegetable challenge (plant → sell) first.'
          : animalDone
        ...
        statusIcon: animalDone ? '✓' : animalLocked ? '🔒' : '▶',
        locked: animalLocked,
        available: !animalLocked && !animalDone,
      });
    }
    ...
    if (cleanTarget > 0 && cleanMessName) {
      ...
      const animalDone = Boolean(levelAnimalComplete);
      const cropsDone = Boolean(levelCropComplete);
      const cleanDone = Boolean(levelCleanComplete) || cleanSold >= cleanTarget;
      const cleanLocked = (!cropsDone || !animalDone) && !cleanDone;
      const cleanStatus = cleanDone
        ? 'COMPLETED'
        : cleanLocked
          ? 'LOCKED'
          : 'AVAILABLE';
      ...
      list.push({
        ...
        title: `${verb} the ${mess}`,
        detail: cleanLocked
          ? 'Locked — finish vegetable and animal challenges first.'
          : cleanDone
        ...
        statusIcon: cleanDone ? '✓' : cleanLocked ? '🔒' : '▶',
        locked: cleanLocked,
        available: !cleanLocked && !cleanDone,
      });
    }
