...
function CardSkeleton() {
  return (
    <article className="relative flex h-full min-h-[22rem] flex-col overflow-hidden rounded-[1.75rem] border border-brand-primary/20 bg-gradient-to-br from-brand-primary/10 via-white to-brand-primary/5 p-6 shadow-sm">
      <div className="h-4 w-28 animate-pulse rounded bg-brand-surface" />
      <div className="mt-4 h-7 w-44 animate-pulse rounded bg-brand-surface" />
      <div className="mt-3 h-20 w-full animate-pulse rounded-2xl bg-brand-surface" />
...
function TakeCourseCard() {
  ...
  return (
    <LearningHubCardShell
      ...
      title="Take a course"
      description="Guided science lessons for Grades 6–9."
      icon={Compass}
      footer={
        <Button
          asChild
          className="h-11 w-full rounded-2xl bg-brand-primary text-base font-semibold text-white shadow-md shadow-brand-primary/25 hover:bg-brand-primary/90"
        >
    ...
    >
      <div className="flex h-full flex-col justify-center gap-2 rounded-2xl border border-brand-primary/15 bg-white/85 p-3.5 backdrop-blur-sm">
        {loadingStats ? (
          <div className="space-y-2">
            <div className="h-4 w-2/3 animate-pulse rounded bg-brand-surface" />
            <div className="h-2.5 animate-pulse rounded-full bg-brand-surface" />
          </div>
        ) : (
          <>
            <div className="flex items-start justify-between gap-2 text-sm font-medium text-brand-text/75">
              <span className="line-clamp-2 min-w-0 flex-1 leading-snug">
                {stats?.label || `Grade ${grade} pathway`}
              </span>
              <span className="shrink-0 text-lg font-black text-brand-primary">
                {percent}%
              </span>
            </div>
            <div
              className="h-2.5 w-full overflow-hidden rounded-full bg-brand-surface"
              role="progressbar"
...
export function PlacementCourseCard() {
  ...
  if (placement.needsAmplitude && !placement.unreachable) {
